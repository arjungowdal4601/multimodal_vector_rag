...
# =============================================================================
def save_page_images(doc, paths: OutputPaths) -> None:
    """Save one rendered PNG per page."""
    for page_no, page in sorted(doc.pages.items()):
        page_image_path = paths.page_images / f"page-{page_no}.png"
...
def collect_assets_from_doc(
    ...
    for element, _level in doc.iterate_items():
        ...
        if isinstance(element, PictureItem):
            counters["picture"] += 1
            image_path = paths.picture_images / f"picture-{counters['picture']}.png"
        ...
        if isinstance(element, TableItem):
            counters["table"] += 1
            image_path = paths.table_images / f"table-{counters['table']}.png"
            image = element.get_image(doc)
            if image is None:
                continue
        ...
        counters["formula"] += 1
        image_path = paths.formula_images / f"formula-{counters['formula']}.png"
        image = element.get_image(doc)
        if image is None:
            continue
...
def doc_processor_with_descriptions(
    ...
    output_root = Path(f"{Path(pdf_path).stem}_doc_assets")
